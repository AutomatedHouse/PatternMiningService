 package com.ahmedehab.patternmining.spmf.algorithms.frequentpatterns.fpgrowth;
...
import com.ahmedehab.patternmining.spmf.patterns.itemset_array_integers_with_count.Itemset;
...
public class AlgoFPGrowth {
	 ...
	 */
	public Itemsets runAlgorithm(List<List<String>> inputDB, String output, double minsupp) throws FileNotFoundException, IOException {
		// record start time
		...
		final Map<Integer, Integer> mapSupport = scanDatabaseToDetermineFrequencyOfSingleItems(inputDB); 
		...
		for(List<String> inputLine: inputDB){
			
			String[] lineSplited = (String[]) inputLine.toArray(new String[inputLine.size()]);
...
//		System.out.println(tree);
	...
	private  Map<Integer, Integer> scanDatabaseToDetermineFrequencyOfSingleItems(List<List<String>> inputDB)
		throws FileNotFoundException, IOException {
	// a map for storing the support of each item (key: item, value: support)
	 Map<Integer, Integer> mapSupport = new HashMap<Integer, Integer>();
	//Create object for reading the input file
	// for each line (transaction) until the end of database
	for(List<String> transaction : inputDB){ 
		
		// take items into database
		String[] lineSplited = (String[]) transaction.toArray(new String[transaction.size()]);
		// for each item
		for(String itemString : lineSplited){  
			// increase the support count of the item
			Integer item = Integer.parseInt(itemString);
